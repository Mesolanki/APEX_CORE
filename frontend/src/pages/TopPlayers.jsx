...
import { FiChevronRight, FiFilter, FiCrosshair, FiZap, FiAlertCircle } from 'react-icons/fi';
...
function TopPlayers() {
    ...
    useLayoutEffect(() => {
        ...
        return () => ctx.revert();
    }, [loading, players, countryFilter, teamFilter, q]);
    ...
    const filtered = useMemo(() => {
        return players.filter((d) => {
            ...
            if (q) {
                const hay = `${d.alias || ''} ${d.car || ''} ${d.team || ''}`.toLowerCase();
                if (!hay.includes(q)) return false;
            }
            return true;
        });
    }, [players, countryFilter, teamFilter, q]);
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center text-cyan-500 font-mono relative overflow-hidden">
                ...
                <FiZap className="text-4xl animate-pulse mb-6 text-cyan-600 drop-shadow-[0_0_15px_rgba(6,182,212,0.8)]" />
            ...
            <div className="relative max-w-7xl mx-auto px-6 z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filtered.map((p, i) => (
                        ...
                        >
                            <Link to={`/drivers/${encodeURIComponent(p.rank)}`} className="block h-full">
                                <div className="interactive-card group relative bg-black border border-gray-800 h-[420px] flex flex-col overflow-hidden hover:border-cyan-600/50 transition-colors cursor-none">
                                    
                                    {/* Image Section */}
                                    <div className="h-[60%] overflow-hidden relative grayscale group-hover:grayscale-0 transition-all duration-500">
                                        <img 
                                            src={p.portrait || 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80'} 
                                            alt={p.alias}
                                            className="w-full h-full object-cover scale-105 group-hover:scale-100 transition-transform duration-700" 
                                        />
                                        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent" />
                                        
                                        <div className="absolute top-4 left-4 bg-cyan-600 text-white text-[9px] font-black px-2 py-1 tracking-widest uppercase shadow-[0_0_10px_rgba(6,182,212,0.5)]">
                                            RANK {p.rank < 10 && p.rank > 0 ? `0${p.rank}` : p.rank}
                                        </div>
                                    ...
                                    <div className="flex-1 p-5 flex flex-col justify-between bg-gradient-to-b from-black to-[#050505] relative z-10 border-t border-gray-900 group-hover:bg-[#0a0a0a] transition-colors">
                                        <div>
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-[9px] text-cyan-500 font-bold tracking-widest uppercase">{p.team?.replace(/_/g, ' ') || 'SOLO'}</span>
                                                {p.status === 'Active' ? <div className="w-1.5 h-1.5 bg-green-500 rounded-full shadow-[0_0_8px_rgba(34,197,94,0.8)]" title="Active" /> : <div className="w-1.5 h-1.5 bg-cyan-500 rounded-full" title="Inactive/Banned" />}
                                            </div>
                                            <h3 className="text-xl font-black uppercase text-white leading-tight tracking-tight">{p.alias?.replace(/_/g, ' ')}</h3>
                                            <p className="text-[10px] text-gray-500 uppercase mt-1 font-bold">{p.car?.replace(/_/g, ' ')}</p>
                                        </div>
                                        ...
                                        <div className="flex justify-between items-end pt-4 border-t border-gray-800 mt-4">
                                            <div>
                                                ...
                                                <span className="font-black text-sm text-gray-300 group-hover:text-white transition-colors">{p.winRate || 'N/A'}</span>
                                            </div>
                                            <span className="text-[9px] text-gray-500 uppercase tracking-widest font-bold group-hover:text-cyan-500 transition-colors flex items-center gap-1">
                ...
                {!loading && players.length === 0 && (
                    <div className="border border-dashed border-gray-800 py-20 text-center">
                        <p className="text-gray-600 text-xs uppercase tracking-widest font-bold">DATABASE EMPTY. NO PLAYER RECORDS FOUND.</p>
                    </div>
                )}

                {!loading && players.length > 0 && filtered.length === 0 && (
                    <div className="border border-cyan-900/30 bg-cyan-900/10 py-16 text-center rounded-sm mt-8">
                        <FiAlertCircle className="mx-auto text-cyan-500 text-2xl mb-3" />
                        <p className="text-cyan-500/80 text-[10px] font-bold uppercase tracking-[0.2em]">
